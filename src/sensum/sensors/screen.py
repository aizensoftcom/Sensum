...
class ScreenSensor:
    """Optional low-cost screen-change detector.

    It emits a semantic *change event* only when enough pixels changed. It deliberately does not
    send frames to an LLM. Applications can attach a local vision model after this gate.
    """
    ...
    async def events(self) -> AsyncIterator[SensoryEvent]:
        try:
            ...
            raise RuntimeError(
                "ScreenSensor requires optional dependencies: pip install 'sensum-ai[screen]'"
            ) from exc
                ...
                if self._previous is not None:
                    ...
                    if ratio >= self.threshold:
                        ...
                        yield SensoryEvent(
                            ...
                            urgency=0.05,
                            metadata={"monitor": self.monitor, "change_ratio": ratio},
                            tags=["changed"],
                        )
