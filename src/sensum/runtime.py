...
class SensumRuntime:
    ...
    def __init__(self, *, attention: AttentionPolicy | None = None, world: WorldState | None = None) -> None:
    ...
    async def ingest(self, event: SensoryEvent) -> bool:
        ...
        event.metadata.setdefault("attention", {"score": round(decision.score, 5), "significant": decision.significant, "reasons": list(decision.reasons)})
    ...
    async def start(self) -> None:
        if self._running:
            return
        self._running = True
        self._tasks = [asyncio.create_task(self._run_sensor(sensor), name=f"sensum:{sensor.name}") for sensor in self._sensors]
